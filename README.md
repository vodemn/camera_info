...
```dart
if (Platform.isIOS) {
  final cameras = await plugin.iosCameraCapabilities;
  // All fields are non-nullable
  print(cameras.first.equivalentFocalLength);
} else {
  final cameras = await plugin.androidCameraCapabilities;
  // Some fields are nullable — see AndroidCameraLensCapabilities
  print(cameras.first.equivalentFocalLength);
}
```
...
### `CameraLensCapabilities` (shared)

Returned by `getCameraCapabilities()`. Fields are non-nullable only where both platforms guarantee a value.
